...
# def write_html(cp: ChapterPage, filename: str = "output.html") -> None:
#     """Write a ChapterPage to an HTML file with nice formatting."""
#     fn_counts = {
#         "tn": len(cp.footnotes_by_type("tn")),
#         "sn": len(cp.footnotes_by_type("sn")),
#         "tc": len(cp.footnotes_by_type("tc")),
#     }
...
#     <style>
#         body {{ font-family: Georgia, serif; max-width: 900px; margin: 0 auto; padding: 20px; line-height: 1.8; background-color: #f9f7f4; }}
#         h1 {{ color: #333; border-bottom: 3px solid #8b7355; padding-bottom: 10px; }}
#         .metadata {{ background-color: #e8e3dc; padding: 15px; border-radius: 5px; margin-bottom: 20px; }}
#         .metadata p {{ margin: 5px 0; }}
#         .section-heading {{ color: #8b7355; font-size: 1.3em; font-weight: bold; margin-top: 30px; margin-bottom: 15px; border-left: 4px solid #8b7355; padding-left: 10px; }}
#         .verse {{ margin: 15px 0; }}
#         .verse-number {{ font-weight: bold; color: #8b7355; }}
#         .verse-text {{ margin-left: 20px; color: #333; }}
#         .footnote {{ font-size: 0.9em; margin-left: 30px; margin-top: 5px; padding: 8px; background-color: #fff8f0; border-left: 3px solid #d4a574; }}
#         .footnote-type {{ font-weight: bold; color: #8b7355; }}
#         .footnote-text {{ color: #555; margin-top: 3px; }}
#         .stats {{ text-align: center; color: #666; font-size: 0.9em; margin-top: 40px; padding-top: 20px; border-top: 1px solid #ccc; }}
#     </style>
# </head>
# <body>
#     <h1>{cp.title}</h1>
...
#     <div class="metadata">
#         <p><strong>Book/Chapter/Version:</strong> {cp.book} / {cp.chapter} / {cp.version}</p>
#         <p><strong>Source:</strong> <a href="{cp.source_url}" target="_blank">{cp.source_url}</a></p>
#         <p><strong>Sections:</strong> {len(cp.sections)} | <strong>Verses:</strong> {len(cp.all_verses)} | <strong>Footnotes:</strong> {len(cp.all_footnotes)}</p>
#     </div>
# """
    
#     for s_i, section in enumerate(cp.sections, 1):
#         if section.heading:
#             html += f'    <div class="section-heading">Section {s_i}: {section.heading}</div>\n'
        
#         for para in section.paragraphs:
#             for verse in para.verses:
#                 verse_label = f"v{verse.number}" if verse.number is not None else "(line)"
#                 html += f'    <div class="verse">\n'
#                 html += f'        <span class="verse-number">[{verse_label}]</span>\n'
#                 html += f'        <div class="verse-text">{verse.plain_text}</div>\n'
                
#                 for chunk in verse.chunks:
#                     for fn in chunk.footnotes:
#                         fn_type_display = {"tn": "Translator's Note", "sn": "Study Note", "tc": "Textual Criticism"}.get(fn.type, fn.type.upper())
#                         html += f'        <div class="footnote">\n'
#                         html += f'            <span class="footnote-type">[{fn_type_display}]</span>\n'
#                         html += f'            <div class="footnote-text">{fn.text}</div>\n'
#                         html += f'        </div>\n'
...
#     html += f'''
#     <div class="stats">
#         <p>Footnote Summary: TN={fn_counts["tn"]} | SN={fn_counts["sn"]} | TC={fn_counts["tc"]}</p>
#     </div>
# </body>
# </html>
# '''
...
if __name__ == "__main__":
    ...
    # Comment: Write to output.txt
    with open("output2.txt", "w", encoding="utf-8") as _f:
        sys.stdout = _f
        print(f"Scraping {URL} ...\n")
        chapter = scrape(URL)
        print_structure(chapter, show_footnotes=True)
    ...
    # Write to HTML file
    # write_html(chapter, "output.html")
